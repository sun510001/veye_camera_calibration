...
LastEditTime: 2024-06-08 22:16:39
FilePath: /veye_camera_calibration/codes/get_camera_frames.py
Description: Loading camera from /dev/video0 or /dev/video8, but script can only record camera by 1080p15fps
"""
    ...
    try:
        while True:
            ...
            if ret:
                ...
                print(
                    f"Frame: {frame_num}, wxh: {cap.get(cv2.CAP_PROP_FRAME_WIDTH)}x{cap.get(cv2.CAP_PROP_FRAME_HEIGHT)}, fps: {cap.get(5)}"
                )
            else:
            ...
            if press_key == ord("q"):
                ...
                file_path = os.path.join(
                    save_folder, f"capture_{get_current_time()}.jpg"
                )
                ...
                if not in_recording:
                    ...
                    in_recording = True
                    file_path = os.path.join(
                        save_folder, f"capture_{get_current_time()}.mp4"
                    )
                    out = cv2.VideoWriter(
                        file_path, fourcc, cap.get(5), (cam_width, cam_height)
                    )
                    out.write(frame)
                else:
                    out.release()
